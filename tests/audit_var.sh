...
if [ $? -eq 1 ]; then
  cmd_check "nodev option set for /var/tmp ... " "findmnt -n /var/tmp | grep -v nodev"
  cmd_check "nosuid option set for /var/tmp ... " "findmnt -n /var/tmp | grep -v nosuid"
  cmd_check "noexec option set for /var/tmp ... " "findmnt -n /var/tmp | grep -v noexec"
  fi
